# Manifold Fedimint Guardian

## Documentation

- [Guardian setup guide](https://manifold.fedi.xyz/guardian-guide) — setup walkthrough and full video.

## Getting set up

1. Install and start Bitcoin if it is not already running.
2. Run **Set Dashboard Password** and save the password it gives you.
3. Start Manifold Fedimint Guardian and open **Operator Dashboard**.
4. Sign in with your dashboard password, then follow the guardian setup guide to authorize your guardian and complete setup.

## Your dashboard password

Run **Set Dashboard Password** again if you need a new password. This replaces the previous password and briefly restarts a running guardian service.

For help, visit [Fedi Support](https://support.fedi.xyz/).
